...
class Controller
{
    ...
    public function get_file($file_path)
    {
        ...
        if( isset( $file_info['extension']) && stripos(Config::get('zyeditor.unable_suffix'), $file_info['extension']) !== false ){
            echo '该文件不可编辑';
            exit;
        }
        ...
        if( isset( $file_info['extension']) && stripos(Config::get('zyeditor.img_suffix'), $file_info['extension']) !== false ){
            //输出图片
