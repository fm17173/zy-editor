...
{    
        ...
        if($now_user){
            ...
            if(!$user_info){
                $user_info = [
                    'username' => $username,
                    'try_login_num' => 0,
                    'password' => md5(Config::get('zyeditor.default_password'))
                ];
            ...
            if(Config::get('zyeditor.password_attempts_num') && ($user_info['try_login_num'] >= Config::get('zyeditor.password_attempts_num'))){
            ...
            if(md5($password) !== $user_info['password']){
                ...
                $limit_login_message = empty(Config::get('zyeditor.password_attempts_num'))?'':'，剩余尝试次数:'.(Config::get('zyeditor.password_attempts_num')-$user_info['try_login_num']);
    ...
    public static function getLimitDir()
    {
        if(self::session('limit_dirs') !== null){
            ...
            $limit_dirs = Config::get('zyeditor.public_access_dirs',[]);
    ...
    {
        $limit_ips = Config::get('zyeditor.public_access_ips',[]);
        $now_user = self::getUserConfig($username);
    ...
    public static function getUserConfig($username = '')
    {
        ...
        foreach(Config::get('zyeditor.users') as $user){
